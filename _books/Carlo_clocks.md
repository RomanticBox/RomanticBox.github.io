---
layout: book-review
title: 시계와 문명 (Clocks and Culture)
author: 카를로 M. 치폴라 (Carlo M. Cipolla)
cover: assets/img/book_covers/carlo_clocks.jpg
olid: cover # use Open Library ID to fetch cover (if no `cover` is provided)
isbn: 9788994142296 # use ISBN to fetch cover (if no `olid` is provided, dashes are optional)
categories: History, Techniques, Western History, Culture, Civilization
tags:
buy_link: https://product.kyobobook.co.kr/detail/S000001542760
date: 2026-02-28
started: 2026-02-28
finished:
released: 2013
stars:
goodreads_review: 3.82
status: reading
---

TODO
